...
export default {
  ...
  data () {
    return {
      ...
      urlDetail: 'https://route.showapi.com/213-2?showapi_appid=42915&showapi_sign=e5c70803e5744a2aae12df378acf3ed0&musicid=', // 歌曲id搜索
      lyricText: '',
      lyricList: {
  ...
  methods: {
    ...
    renderLyrics () { // 对歌词模块进行渲染
      ...
      this.$http.get(this.urlDetail + this.song.songid).then((response) => {
        ...
        this.$nextTick(() => {
          ...
          console.log(this.lyricList.list)
          this.setListScroll()
    ...
    setLyricHighlight (index) {
      this.lyricList.list[index - 1].on = true // 设置高亮

      if (index - 1) { // 第四句歌词才开始滚动
        this.$refs.lyricsContent.style.transform = `translate3d(0, -${(index - 1) * 34}px, 0)`
      }
    },
    checkLyric (time) {
      ...
      for (let i = 0; i < length; i++) {
        ...
        if (val.totalTime > time) {
          ...
          this.setLyricHighlight(i)
          break
        }
        if (time > this.lyricList.list[length - 1].totalTime) {
          this.setLyricHighlight(length)
        }
      }
    },
        // checkLyric(time) {
